import org.junit.After;
import org.junit.Before;
import org.junit.Test;
import org.openqa.selenium.*;
import org.openqa.selenium.chrome.ChromeDriver;
import org.openqa.selenium.support.ui.Select;

import java.time.Duration;

import static org.junit.Assert.assertEquals;
import static org.junit.Assert.fail;

public class KatalonTest {
    private WebDriver driver;
    private String baseUrl;

    @Before
    public void setUp() throws Exception {
        System.setProperty("webdriver.chrome.driver", "C:\\Users\\asjap\\OneDrive\\Рабочий стол\\Моё\\обучение\\тестирование ПО авг.2023\\GIT\\Pivankova_QA\\Praktika3\\chromedriver.exe");
        driver = new ChromeDriver();
        baseUrl = "https://sandbox.cardpay.com/MI/cardpayment2.html?orderXml=&sha512=998150a2b27484b776a1628bfe7505a9cb430f276dfa35b14315c1c8f03381a90490f6608f0dcff789273e05926cd782e1bb941418a9673f43c47595aa7b8b0d";
        driver.manage().timeouts().implicitlyWait(Duration.ofSeconds(60));
        driver.manage().window().fullscreen();
    }

    @Test
    public void ConfirmadPay() throws Exception {
        driver.get(baseUrl);
        String totalAmount = (String) driver.findElement(By.xpath("//*[@id=\"total-amount\"]")).getText();
        String Currency = (String) driver.findElement(By.xpath("//*[@id=\"currency\"]")).getText();
        String order_number = (String) driver.findElement(By.id("order-number-cnt")).getText();
        WebElement inputCardNumber = driver.findElement(By.id("input-card-number"));
        inputCardNumber.click();
        inputCardNumber.clear();
        inputCardNumber.sendKeys("4000000000000051");
        inputCardNumber.click();
        WebElement inputCardHolder = driver.findElement(By.id("input-card-holder"));
        inputCardHolder.click();
        inputCardHolder.clear();
        inputCardHolder.sendKeys("ASJA P");
        driver.findElement(By.id("card-expires-month")).click();
        new Select(driver.findElement(By.id("card-expires-month"))).selectByVisibleText("12");
        driver.findElement(By.id("card-expires-year")).click();
        new Select(driver.findElement(By.id("card-expires-year"))).selectByVisibleText("2032");
        WebElement inputCardCvc = driver.findElement(By.id("input-card-cvc"));
        inputCardCvc.click();  // тоже самое, что: driver.findElement(By.id("input-card-cvc")).click();
        inputCardCvc.clear();  // тоже самое, что: driver.findElement(By.id("input-card-cvc")).clear();
        inputCardCvc.sendKeys("897");  // тоже самое, что: driver.findElement(By.id("input-card-cvc")).sendKeys("897");
        driver.findElement(By.id("action-submit")).click();
        //driver.findElement(By.id("successButton")).submit(); не нужно при No 3-D Secure, например при  "4000000000000051"


        assertEquals("CONFIRMED", driver.findElement(By.xpath("//div[@id='payment-item-status']/div[2]")).getText());
        assertEquals(order_number, "Order number: " + driver.findElement(By.xpath("//*[@id=\"payment-item-ordernumber\"]/div[2]")).getText());
        assertEquals(Currency + "   " + totalAmount, driver.findElement(By.xpath("//*[@id=\"payment-item-total\"]/div[2]")).getText());

    }

    @Test
    public void invalidCardNumber() throws Exception {
        driver.get(baseUrl);
        WebElement inputCardNumber = driver.findElement(By.id("input-card-number"));
        inputCardNumber.click();
        inputCardNumber.clear();
        inputCardNumber.sendKeys("0000000000000000");
        inputCardNumber.click();
        WebElement inputCardHolder = driver.findElement(By.id("input-card-holder"));
        inputCardHolder.click();
        inputCardHolder.clear();
        inputCardHolder.sendKeys("ASJA P");
        driver.findElement(By.id("card-expires-month")).click();
        new Select(driver.findElement(By.id("card-expires-month"))).selectByVisibleText("12");
        driver.findElement(By.id("card-expires-year")).click();
        new Select(driver.findElement(By.id("card-expires-year"))).selectByVisibleText("2032");
        WebElement inputCardCvc = driver.findElement(By.id("input-card-cvc"));
        inputCardCvc.click();  // тоже самое, что: driver.findElement(By.id("input-card-cvc")).click();
        inputCardCvc.clear();  // тоже самое, что: driver.findElement(By.id("input-card-cvc")).clear();
        inputCardCvc.sendKeys("897");  // тоже самое, что: driver.findElement(By.id("input-card-cvc")).sendKeys("897");
        driver.findElement(By.id("action-submit")).click();

        assertEquals("Card number is not valid", driver.findElement(By.xpath("//*[@id=\"card-number-field\"]/div/label")).getText());
    }

    @After
    public void tearDown() throws Exception {
        driver.quit();
    }

}

